...
def get_stratified_sample(ds_df, count, p_group = 0.2, p_polarity = 0.2, p_submitter = 0.3,  p_matrix = 0.2 ):
    ...
    cols = [
        ('matrix_solvent',int(p_matrix * count)),
        ('source', 2),
        ('analyzer', 2),
        ('rp_range', 3),
        ('submitter', int(p_submitter * count)),
        ('group', int(p_group * count)),
        ('polarity', int(p_polarity * count)),
        #('organism', 50),
        #('organism_part', 20),
        #('condition', 20)
    ]
