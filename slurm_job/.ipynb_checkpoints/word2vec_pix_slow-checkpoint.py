...
from pixel_dataframes import create_pixel_df #That's new
...
from numpy import exp, log, dot, zeros, outer, random, dtype, float32 as REAL,\
    ...
    ndarray, empty, sum as np_sum, prod, ones, ascontiguousarray, vstack, logaddexp, random, nan as np_nan
...
logger = logging.getLogger(__name__)
 # try:
 #     from word2vec_inner.word2vec_inner import train_batch_sg, train_batch_cbow_pix
 #     from word2vec_inner.word2vec_inner import score_sentence_sg, score_sentence_cbow
 #     from word2vec_inner.word2vec_inner import FAST_VERSION, MAX_WORDS_IN_BATCH
 # except ImportError:
...
def train_batch_cbow_pix(model, word_id_lists, alpha, work=None, neu1=None, compute_loss=False, pix_perc = 1.0):
    result = 0
    for word_id_list in word_id_lists:
        ...
        for pos, word in enumerate(word_vocabs):
            ...
            l1 = np_sum(model.wv.syn0[word2_indices], axis=0)  # 1 x vector_size #replaced attribute syn0 by vectors
...
def train_batch_cbow(model, sentences, alpha, work=None, neu1=None, compute_loss=False):
    ...
    result = 0
    for sentence in sentences:
        # SEVERAL RANDOM WORDS FROM THE WHOLE SENTENCE ARE CHOOSEN
        word_vocabs = [model.wv.vocab[w] for w in sentence if w in model.wv.vocab and
                       model.wv.vocab[w].sample_int > model.random.rand() * 2**32]
        print('SENTENCE:', sentence)
        for wvc in word_vocabs:
            print('WORD_VOCABS',str(wvc))
        for pos, word in enumerate(word_vocabs):
            reduced_window = model.random.randint(model.window)  # `b` in the original word2vec code
            start = max(0, pos - model.window + reduced_window)
...
def score_sentence_cbow(model, sentence, work=None, neu1=None):
    ...
    for pos, word in enumerate(word_vocabs):
        ...
        word2_indices = [word2.index for pos2, word2 in window_pos if (word2 is not None and pos2 != pos)]
        l1 = np_sum(model.wv.syn0[word2_indices], axis=0)  # 1 x layer1_size #replaced syn0 by vectors
        if word2_indices and model.cbow_mean:
            l1 /= len(word2_indices)
...
def score_sg_pair(model, word, word2):
    l1 = model.wv.syn0[word2.index] #replaced syn0 by vectors
...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(
            self, corpus, ds_ids = None, train=None, dict_size=None, ind_name=None, size=100, alpha=0.025, window=5, min_count=5,
            max_vocab_size=None, sample=1e-3, seed=1, workers=8, min_alpha=0.0001,
            sg=0, hs=0, negative=5, cbow_mean=1, hashfxn=hash, iter=5, null_word=0, 
            trim_rule=None, sorted_vocab=1, batch_words=MAX_WORDS_IN_BATCH, compute_loss=False, 
            fdr=0.51, int_per=0.5, pix_per=1., quan=99.):
        """
        ...
        if FAST_VERSION == -1:
            logger.warning('Katyas code: Slow version of {0} is being used'.format(__name__))
        else:
            logger.debug('Fast version of {0} is being used'.format(__name__))
        
        self.size = size # changed this one
        self.ds_ids = ds_ids # changed this one
        self.wv = KeyedVectors(self.size) #changed this one
        self.sg = int(sg)
        self.cum_table = None  # for negative sampling
        self.vector_size = int(self.size)
        self.layer1_size = int(self.size)
        if self.size % 4 != 0:
        ...
        self.train = train # added this one

        #if self.ds_ids is not None: #originally ind_name is not None and train is not None
        self.build_vocab_pix(ds_ids = self.ds_ids, ds_dir = self.train, dict_size = None, ind_name = ind_name, fdr=fdr, int_per=int_per, pix_per=pix_per, quan=quan)
    ...
    def initialize_word_vectors(self):
        self.wv = KeyedVectors(vector_size=self.size) #changed this one
    ...
    def make_cum_table(self, power=0.75, domain=2**31 - 1):
        ...
        for word_index in xrange(vocab_size):
            train_words_pow += self.wv.vocab[self.wv.index2word[word_index]].count**power #vocab became key_to_index
        cumulative = 0.0
        for word_index in xrange(vocab_size):
            cumulative += self.wv.vocab[self.wv.index2word[word_index]].count**power #vocab became key_to_index
    ...
    def build_vocab_pix(self, ds_ids = None, ds_dir=None, dict_size=None, ind_name=None, min_count=None, sample=None, fdr=None,
                        int_per=None, pix_per=None, quan=None):
        logger.info("Building vocabulary for %s from %s", ind_name, ds_dir)
        
        self.corpus_count = 0


        
        """Calculate word frequencies and number of pixels given as examples."""
        #ind_freq = dict.fromkeys(range(0, dict_size),0) # katyas
        ...
        if ds_dir != None:
            for f in os.listdir(ds_dir):
                ...
                filt_df = ds_df[ds_df[ion_names] > int_thresh]
                filt_df = filt_df.astype(pd.SparseDtype("int", np_nan))
                    
                # counting the unique occurences of intensites over threshold for each ion. Probably won't need it
                #unique_ion_freq = {self.ind_name :len(np.unique(np.array(ion_intensities_dict[ion_name]))) for ion_name in ion_intensities_dict}   

                freq_counts = dict(filt_df[ion_names].count()) # Here, we count any pixel above threshold. Alternatively, we could also sum 
                                                               # the pixel intensities over threshold to take the magnitude of the
                                                               # intensities into greater consideration

                for ion in freq_counts:
                    ind_freq[ion] = ind_freq.get(ion, 0) +freq_counts[ion] # update the 'more global' ind_freq 
                                                                            # when the ion is not present yet it is added with default value 0
                    self.corpus_count += round(len(filt_df.dropna(how = 'all').drop_duplicates()) * pix_per)

        else:
            for ds_id in ds_ids:
                ds_df, all_ions = create_pixel_df(dsid=ds_id)
        ...
        ind_freq_res = {i: f for i,f in ind_freq.items() if f >= min_count} # Rarely occuring ions are filtered out
        # Note, that this does not take the intensities magnitude into account

        retain_total = sum(itervalues(ind_freq_res))
        ...
        for ion in ind_freq_res:
            v = ind_freq_res[ion] # frequency of the ion
            word = str(ion) # katya used an index instead of the ion name and therefore
                            # needed to define it as a string (redundant for us)

            self.wv.vocab[word] = Vocab(count=v, index = len(self.wv.index2word))
            # This is the wordvector, index2word replaced by index2key
            self.wv.index2word.append(word)                    

            word_probability = (sqrt(v / threshold_count) + 1) * (threshold_count / v) # how does this come about?
            if word_probability < 1.0:
                downsample_total += v
            self.wv.vocab[word].sample_int = int(round(word_probability * 2**32))
        logger.info("collected %i word types from a corpus of %i total examples", len(ind_freq_res), self.corpus_count)
        ...
        if self.null_word:
            ...
            self.wv.index2word.append(word)
            self.wv.vocab[word] = v #vocab dict became key2index
        self.reset_weights()
    ...
    def _do_train_job_pix(self, word_lists, alpha, inits):
        work, neu1 = inits
        tally = 0
        if self.sg:
            tally += train_batch_sg_pix(self, word_lists, alpha, work, self.compute_loss)
        else:
            tally += train_batch_cbow_pix(self, word_lists, alpha, work, neu1, self.compute_loss)
        #logger.warning("IMPORTANT INFO: tally %i, self._raw_word_count(word_lists) %i", tally, self._raw_word_count(word_lists))
        return tally, self._raw_word_count(word_lists)
    ...
    def train_pix(self, corpus, total_examples=None, total_words=None,
              epochs=None, start_alpha=None, end_alpha=None,
              word_count=0,
              queue_factor=2, report_delay=1.0, compute_loss=None): # for testing porpuses, we changed queuefactor from 2 to 0 (i.e. infinite queue size)

        #total_words = total_words or self.total_words
        total_examples = total_examples or self.total_examples

        if (self.model_trimmed_post_training):
            raise RuntimeError("Parameters for training were discarded using model_trimmed_post_training method")
        if FAST_VERSION < 0:
            warnings.warn("This is katya's code: C extension not loaded for Word2Vec, training will be slow. "
        ...
        logger.info(
            ...
            "using sg=%s hs=%s sample=%s negative=%s window=%s for %s iterations. BTW: We are using train_pix.",
            self.workers, len(self.wv.vocab), self.layer1_size, self.sg,
            self.hs, self.sample, self.negative, self.window, self.iter)
    ...
    #this is commented out in comparision to the normal model.train
        ...
        if epochs > 1:
            corpus = utils.RepeatCorpusNTimes(corpus, epochs)
        ...
        def worker_loop():
            ...
            while True:
                ...
                corpus, alpha = job # sentences was replaced by corpus
                tally, raw_tally = self._do_train_job_pix(corpus, alpha, (work, neu1))
                progress_queue.put((len(corpus), tally, raw_tally))  # report back progress
                jobs_processed += 1
        ...
        def job_producer():
            ...
            for word_list in corpus: # corpus instead of sentences
                list_length = self._raw_word_count([word_list])

                # can we fit this sentence into the existing job batch?
                if batch_size + list_length <= self.batch_words:
                    # yes => add it to the current job
                    job_batch.append(word_list) 
                    batch_size += list_length # list_length instead of sentence length
                else:
                    ...
                    job_batch, batch_size = [word_list], list_length # word list and lind_length instead of sentence and sentence length
    ...
    def load(cls, *args, **kwargs):
        ...
        if not hasattr(model, 'syn0_lockf') and hasattr(model, 'vectors'):
    ...
    def _load_specials(self, *args, **kwargs):
        ...
        if not hasattr(self, 'wv'):
            wv = KeyedVectors()
            wv.syn0 = self.__dict__.get('vectors', [])
...
class PixelCorpus(object): #Replaced this one
    def __init__(self, fdr_thresh=0.1, pix_per=0.5, int_per=0.5, window=5, quan=99., ind_name=None, ds_dir=None, ds_ids=None):
        self.fdr = fdr_thresh
        self.ds_dir = ds_dir
        self.p = pix_per
        self.i = int_per
        ...
        if self.ds_dir != None:
            for f in os.listdir(self.ds_dir):
                ...
                for _, c_row in sampled_coord_df.iterrows():
                    ...
                    # find rows corresponding to pixels auround the sampled pixel coordinates
                    ...
                    exp_inds = []
                    ind_counts = dict(window_rows[ion_names].value_counts())
            ...
            for ds_id in self.ds_ids:
                ds_df, all_ions = create_pixel_df(dsid = ds_id, fdr=self.fdr)
                ...
                # fiter out rows based on intensity and quantile param
                int_thresh = percentile(ds_df[ion_names], self.q) * self.i # this now uses a general intensity threshold,
                                                               # could also use an ion specific one
                filt_df = ds_df[ds_df[ion_names] > int_thresh]
                filt_df = filt_df.astype(pd.SparseDtype("int", np_nan))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ion_df", help="Ion data msgpack", default=None) 
    parser.add_argument("-input_ds", help="Dataset dataframe as csv", type = str,  default=None) # added this one
    parser.add_argument("-formula_df", help="Ion data msgpack", default=None)
    ...
    parser.add_argument("-output", help="Use file OUTPUT to save the resulting word vectors",type=str, default = 'test_output_')
    ...
    parser.add_argument("-threads", help="Use THREADS threads (default 8)", type=int, default=8)
    ...
    # if  args.ion_df:
        ...
        # dict_df_path = path.join(base_path, ind_name+'_df.msgpack')
        # dict_df = pd.read_msgpack(dict_df_path)
        # dict_size = len(dict_df.index)

        # ind_name = ind_name + '_ind'

    if args.train == None:
        if args.input_ds:
            input_ds = pd.read_pickle(args.input_ds)
            ds_ids = input_ds.ds_id.tolist()[:10] # take 10 dataset ids for debugging
        elif args.ds_ids:
            ds_ids = args.ds_ids
        else:
            print('No input.')
    ...
    corpus = PixelCorpus(ds_ids = ds_ids, ds_dir = args.train, ind_name = args.ind_name, fdr_thresh = args.fdr,
                         pix_per = args.pix_per, int_per = args.int_per, window = args.window, quan = args.quan)

    if args.ind_name == 'ion_ind': ind_dict = args.ion_df
    elif args.ind_name == 'formula_ind': ind_dict = args.formula_df

    # if not args.example:
    #     ind_df = pd.read_msgpack(ind_dict)
    #     dict_size = len(ind_df.index)

    model = Word2Vec(
        corpus=corpus,ds_ids = ds_ids, train=args.train, dict_size=None, ind_name= args.ind_name, 
        size=args.size, min_count=args.min_count, workers=args.threads,
    ...
    with open(f'{args.output}.model', 'wb') as saved_model:
        model.save(saved_model)

   # if args.output:
   #     outfile =  args.output
   #     model.wv.save_word2vec_format(outfile, binary = args.binary) # binary = args.binary
   #     #model.save(outfile)
   # else:
   #     outfile = args.train or 'output'
   #     model.save(outfile + '.model.txt')
        
    if args.binary == 1:
        model.wv.save_word2vec_format(f'{args.output}' + '.model.bin', binary=True) 
        #model.wv.save(outfile + '.model.bin') # changed this myself
    else:
        model.wv.save_word2vec_format(f'{args.output}' + '.model.txt', binary=False)
        #model.wv.save(outfile + 'wv.model.txt') # saving the word vectors, changed this myself
