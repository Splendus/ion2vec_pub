...
try:
    from word2vec_inner import train_batch_sg, train_batch_cbow_pix
    ...
    def build_vocab_pix(self, ds_ids = None, ds_dir=None, dict_size=None, ind_name=None, min_count=None, sample=None, fdr=None,
        ...
        if ds_dir != None:
            ...
            for ds_id in ds_ids:
                ...
                filt_df = ds_df
                filt_df[ion_names] = ds_df[ion_names][ds_df[ion_names] > int_thresh]
                filt_df = filt_df.astype(pd.SparseDtype("int", np_nan)) # int should be replaced by float
                                                                        # when we care about the exact value of intensity
        ...
        if self.ds_dir != None:
            for f in os.listdir(self.ds_dir):
                try:
                    ...
                    continue
                all_ions = ds_df.drop(columns=['y','x']).columns.tolist() # list of all ions in ds_df
                logger.info("ds_df pixel size %i ", len(ds_df[['x', 'y']].drop_duplicates().index))
...
if __name__ == "__main__":
    ...
    corpus = PixelCorpus(ds_ids = ds_ids, ds_dir = args.train, ind_name = ind_name, fdr_thresh = args.fdr,
                         pix_per = args.pix_per, int_per = args.int_per, window = args.window, quan = args.quan)

    model = Word2Vec(
        corpus=corpus,ds_ids = ds_ids, train=args.train, dict_size=None, ind_name= ind_name, 
