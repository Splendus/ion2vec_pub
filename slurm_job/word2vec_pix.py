...
class Word2Vec(utils.SaveLoad):
    ...
    def _do_train_job_pix(self, word_lists, alpha, inits):
        ...
        tally = 0
        if self.sg:
            tally += train_batch_sg(self, word_lists, alpha, work, self.compute_loss)
        else:
...
class PixelCorpus(object):
    def __init__(self, fdr_thresh=0.1, pix_per=0.5, int_per=0.5, window=5, quan=99., ind_name=None, ds_dir=None, ds_ids=None, stride = 1, shuff = 0):
        self.fdr = fdr_thresh
        self.ds_dir = ds_dir
        self.p = pix_per
        self.i = int_per
    ...
    def __iter__(self):
        ...
        for f in os.listdir(self.ds_dir):
            ...
            for _, c_row in sampled_coord_df.iloc[::self.stride].iterrows():
                ...
                if self.shuff == 1:
                    shuffle(exp_inds) # shuffle ions in window
                yield exp_inds
...
if __name__ == "__main__":
    ...
    parser.add_argument("-quan", help="Quantile parameter", type = float, default = 99.)
    ...
    corpus = PixelCorpus(ds_ids = ds_ids, ds_dir = args.train, ind_name = ind_name, fdr_thresh = args.fdr,
                         ...
                         stride = args.stride, shuff = args.shuffle)
